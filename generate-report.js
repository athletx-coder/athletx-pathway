  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 8000,
        // Explicit cache breakpoint on the static system prompt (instructions +
        // JSON schema) — identical on every call, only the student's answers
        // in the user message change. This caches the instruction block so
        // back-to-back assessments (e.g. a club running several kids through
        // it in one sitting) read it at 10% of normal input price instead of
        // paying full price each time. No effect if calls are spread out
        // beyond the 5-minute cache window.
        system: [
          {
            type: 'text',
            text: systemPrompt,
